...
def transicao_estendida(dfa, estado_atual, palavra):
    ...
    Returns:
        Tupla (estado_final, lista_de_passos)
    """
    resultado = []
    resultado.append(f"δ*({estado_atual}, ε) = {estado_atual}")
    ...
    for i, simbolo in enumerate(palavra, start=1):
        ...
        if estado_atual is None:
            resultado.append(f"δ*({estado_anterior}, {palavra[:i]}) = None (transição indefinida)")
            break
            
        prefixo = palavra[:i-1] if palavra[:i-1] != "" else "ε"
        resultado.append(
            f"δ*({dfa['estado_inicial']}, {palavra[:i]}) = "
            f"δ(δ*({dfa['estado_inicial']}, {prefixo}), {simbolo}) = "
            f"δ({estado_anterior}, {simbolo}) = {estado_atual}"
        )
